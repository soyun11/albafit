...
## 추가 개선 (사용자 UI 재검토 요청, 같은 날)

커밋 직후 사용자가 "패널 헤더에 제목·링크·버튼이 몰려서 산만하다"고 지적해 두 가지를 더 손봤다.

| 논점 | 결정 | 이유 |
|---|---|---|
| "+ 새 알바 초대" 위치 | 패널 헤더의 파란 버튼 대신, 알바 목록 표 맨 아래에 연한 회색 "추가" 행으로 | Notion/Linear류에서 흔한 "목록의 자연스러운 다음 항목" 패턴. 헤더에 파란 버튼+파란 링크 두 개가 나란히 있어 뭐가 주 액션인지 헷갈리던 문제 해소. |
| "정정 이력 모아보기" 위치 | 대시보드(알바 목록)에서 빼서 "기준 관리" 화면의 "기준 재설정" 버튼 옆으로 이동 | 정정 이력은 "채점 품질 관리"(루브릭 정확도 확인)에 가깝지, "알바 명단 관리"와는 성격이 다르다. 기준 관리 화면이 이미 루브릭 정확도를 다루는 곳이라 더 자연스러운 자리. |

구현: `OwnerDashboard.jsx`에서 헤더 액션 제거하고 `<tbody>` 마지막에 `.add-staff-row`(colSpan 4, 연한 회색, 클릭 시 `invite`로 이동) 추가. `RubricApproval.jsx`의 "기준 재설정" 버튼 옆에 `.link-inline` 스타일의 "정정 이력 모아보기" 링크 추가, `CorrectionHistory.jsx`의 `current`를 `"dashboard"`→`"rubricManage"`로 변경(진입 경로가 바뀌었으므로 활성 표시도 따라감).

브라우저 확인: 대시보드 표 맨 아래 회색 "+ 새 알바 초대" 행 클릭 → 초대 폼 정상 진입. 기준 관리 화면에서 "정정 이력 모아보기" 클릭 → 정정 이력 화면 정상 진입, nav에서 "기준 관리"가 활성 표시. 콘솔 에러 없음.
