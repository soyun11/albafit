...
## 테스트 종류 — 현업에서는 보통 이렇게 나눈다

아래로 갈수록 많이, 위로 갈수록 적게 두는 게 일반적(테스트 피라미드).

- **단위 테스트(Unit)** — 함수 하나, 가장 좁은 범위. `sessionTurns.test.js`가 여기 해당. 전체의 70~80%를 이 레벨로 채우는 게 보통.
- **통합 테스트(Integration)** — 여러 조각이 실제로 맞물려 도는지 확인. 이 프로젝트로 치면 라우트 단위 테스트 — 예를 들어 `POST /api/sessions/:id/turns`를 호출했을 때 (mock된) Gemini 응답을 받아서 `session_turns`에 실제로 잘 저장되는지. DB는 진짜(테스트용) 쓰되 외부 API만 mock하는 식. 내일 루브릭 재사용성에서 "두 번째 매장이 제출하면 Gemini가 실제로 호출 안 되는지" 검증하는 것도 이 레벨.
- **E2E(End-to-End)** — 브라우저를 실제로 띄워 사용자처럼 클릭·입력하며 전체 흐름 확인(Playwright/Cypress). 느리고 깨지기 쉬워서 핵심 흐름 몇 개만 둔다. 4주 챌린지 범위에서는 시간 대비 이득이 적어서 아마 생략.

## 실제로 테스트를 짜보면서 걸렸던 것들

`sessionTurns.test.js`를 직접 써보면서 나온 실수들 — 코드 리뷰 없이 혼자 짰으면 그냥 통과했을 수도 있는 것들이라 기록해둔다.

- **테스트 데이터가 서로 구분돼야 의미가 있다.** `buildConversationHistory` 테스트에서 두 시도의 `staffAnswer`를 둘 다 `'...'`로 똑같이 넣었더니, 결과 배열의 길이나 `sender` 순서는 맞아도 "값 자체가 제대로 조합됐는지"는 전혀 증명이 안 됐다. `retryCount:0`은 `'죄송합니다'`, `retryCount:1`은 `'10분 정도 걸려요'`처럼 서로 다른 값을 넣고 `result[1].text`/`result[2].text`까지 확인해야 진짜 검증이 된다.
- **오타는 테스트를 돌려야 드러난다.** `staffAnswer`를 `staffAnswwer`로 오타 냈는데, 문법 에러가 아니라서(그냥 존재하지 않는 필드) 코드만 봐서는 못 잡고, 함수가 `undefined`를 돌려주는 걸 보고서야 알아챘다. 이게 정확히 테스트가 잡아주는 종류의 실수.
- **인덱스 기반 assertion은 함수 로직을 먼저 정확히 따라가야 한다.** `result[2].sender`를 `'customer'`로 잘못 기대했던 적이 있었는데, `buildConversationHistory`가 손님 발화를 턴마다 한 번만 넣고 그다음 모든 시도를 `staff`로 넣는다는 걸 다시 확인하고서야 고쳤다. "몇 번째 인덱스에 뭐가 와야 하는지"는 감이 아니라 소스 코드를 보고 정해야 한다.

## 관련 개념

- [두 에이전트 요청 흐름](../ai/two-agent-flow.md) — 지금은 순수 함수만 테스트했지만, 이 흐름에 붙는 라우트를 통합 테스트로 검증할 때 참고
