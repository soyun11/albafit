...
## 추가: 상단 nav 독립 탭 분리 + 이름 변경 "AI 채점 검토" (2026-07-22)

"정정 이력 모아보기"는 지금까지 사장님 대시보드의 "기준 관리" 탭(`RubricApproval.jsx`) 상단에 링크 버튼 하나로만 붙어 있었다. 사용자가 두 기능(루브릭 자체를 고치는 것 vs 알바 세션의 AI 채점을 점검·교정하는 것)이 성격이 다르다고 지적해 상단 nav의 독립 탭으로 분리한다.

- **이름 변경**: "정정 이력 모아보기" → **"채점 검토 이력"**. 처음엔 `FeedbackReport.jsx`의 캘리브레이션 진입 버튼("AI 채점 검토하기")과 용어를 맞춰 "AI 채점 검토"로 바꿨는데, 그러면 "교정을 직접 남기는" 화면(리포트 → "AI 채점 검토하기" → `TurnCalibrationReview.jsx`)과 "이미 남긴 교정을 모아보기만 하는" 화면(nav → `CorrectionHistory.jsx`, 읽기 전용)이 거의 같은 이름이 돼서 사장님이 nav 탭을 눌렀을 때 교정을 남길 수 있는 화면으로 착각할 수 있다는 문제가 나왔다. 그래서 "이력"을 붙여 읽기 전용 모아보기 화면임을 명확히 구분한다 — "정정" 대신 "채점 검토"라는 용어는 유지해 리포트 쪽 버튼과의 연관성은 남긴다.
- **화면·API는 새로 만들지 않음**: `CorrectionHistory.jsx`, `GET /api/stores/me/corrections`, `screenAccess.js`의 `OWNER_SCREENS` 접근 가드, `App.jsx`의 `effectiveScreen === 'correctionHistory'` 라우팅 분기가 이미 전부 구현·동작 중이었다 — 라우터 없는 `useState` 문자열 화면 전환 구조라 "새 화면을 뚫는" 작업이 아니라 `AppNav.jsx`의 `OWNER_LINKS`에 진입점 하나 추가하는 작업으로 끝난다.
- **탭 위치**: "대시보드 → 기준 관리 → AI 채점 검토" 순서(기준 관리 바로 다음). 캘리브레이션 대상이 루브릭이라 연관성이 높다는 이유.
- **기존 링크 버튼 제거**: nav에 전용 탭이 생겼으니 `RubricApproval.jsx`의 "정정 이력 모아보기 →" 링크는 중복 진입 경로라 제거하고 "기준 재설정" 버튼만 남긴다.

**구현·검증**: `AppNav.jsx`의 `OWNER_LINKS`에 `{ key: 'correctionHistory', label: '채점 검토 이력' }` 추가, `CorrectionHistory.jsx`의 `current`/`h1`을 새 라벨로 수정, `RubricApproval.jsx`에서 링크 버튼 제거 — 3개 파일 전부 기존 화면·라우팅·가드를 그대로 재사용해 새 코드 없이 끝남. 루트 `npm run lint` 통과, `screenAccess.test.js`(12개) 통과. 브라우저(`claude-in-chrome`)로 실제 로그인 세션에서 "대시보드 → 기준 관리 → 채점 검토 이력" 순서로 nav가 뜨는 것, 탭 클릭 시 active 스타일 전환과 `CorrectionHistory` 화면 진입, "기준 관리" 화면에서 기존 링크 버튼이 사라지고 "기준 재설정" 버튼만 남은 것을 확인. 콘솔 에러 없음. (라벨을 "AI 채점 검토" → "채점 검토 이력"으로 한 번 더 수정한 경위는 위 본문 참고 — 리포트 화면의 "AI 채점 검토하기" 버튼과 이름이 겹쳐 헷갈린다는 지적을 반영.)
